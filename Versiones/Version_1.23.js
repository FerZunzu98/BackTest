...
async function Bucle (resolution){
    ...
    let stat_BJ_dir = SepararDireccion(jugadas_bajo_risk);

    console.log("Estadísticas largos  ")
    console.log(Estadisticas(stat_BJ_dir.largos));
    console.log("Estadísticas cortos")
    console.log(Estadisticas(stat_BJ_dir.cortos));


    // console.log("Estadísticas Jugadas de bajo riesgo");
    // console.log("Jugadas total:"+stat_BJ.jugadas_total);
    // console.log("Porcentaje acierto: "+stat_BJ.porcentaje_acierto);
    // console.log("Ganancia media: "+stat_BJ.ganancia_media);
    // console.log("Perdida media: "+stat_BJ.perdida_media);
    // console.log("Benefio por operacion: "+stat_BJ.beneficio_operacion);
    // console.log("Riesgo promedio: "+ RiesgoPromedio(jugadas_bajo_risk));
    
    // let stat_AJ = Estadisticas(jugadas_alto_risk);

    // console.log("Estadísticas Jugadas de Alto riesgo");
    // console.log("Jugadas total:"+stat_AJ.jugadas_total);
    // console.log("Porcentaje acierto: "+stat_AJ.porcentaje_acierto);
    // console.log("Ganancia media: "+stat_AJ.ganancia_media);
    // console.log("Perdida media: "+stat_AJ.perdida_media);
    // console.log("Benefio por operacion"+stat_AJ.beneficio_operacion);
    // console.log("Riesgo promedio: "+ RiesgoPromedio(jugadas_alto_risk));
...
async function VBS_1 (array_datos,accion){
    ...
    for(i=20; i < array_datos.c.length; i++){
        ...
        let high_1 = array_datos.h[i-1];
        let high_2 = array_datos.h[i-2];
        let high_3 = array_datos.h[i-3];
        let high_4 = array_datos.h[i-4];
        let high_5 = array_datos.h[i-5];
        let high_6 = array_datos.h[i-6];
        let high_7 = array_datos.h[i-7];
        let high_8 = array_datos.h[i-8];    
        let high_9 = array_datos.h[i-9];    

        let low = array_datos.l[i];
        let low_1 = array_datos.l[i-1];
        let low_2 = array_datos.l[i-2];
        let low_3 = array_datos.l[i-3];
        let low_4 = array_datos.l[i-4];
        let low_5 = array_datos.l[i-5];
        let low_6 = array_datos.l[i-6];
        let low_7 = array_datos.l[i-7];
        let low_8 = array_datos.l[i-8];    
        let low_9 = array_datos.l[i-9];  
        ...
        let alto = Mayor([high,high_1,high_2,high_3,high_4,high_5,high_6,high_7,high_8,high_9]);
        let bajo = Menor([low,low_1,low_2,low_3,low_4,low_5,low_6,low_7,low_8,low_9]);
        ...
        if(MM20 > MM20_10 && MM20_10 > MM20_20 && MM8 > MM8_3 && MM8_3 > MM8_5 && high < high_1 && high_1 < high_2){
            ...
            if(activacion){
                ...
                if(activacion.entrada + (activacion.entrada - activacion.stop) <= alto){
        ...
        if(MM20 < MM20_10 && MM20_10 < MM20_20 && MM8 < MM8_3 && MM8_3 < MM8_5 && low > low_1 && low_1 > low_2){
            ...
            if(activacion){
                ...
                if(activacion.entrada - (activacion.stop - activacion.entrada) >= bajo){
                    jugadas.push(activacion);
                }
            }
        };
